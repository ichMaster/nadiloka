...
- `/release-version 0.2.0 Add the Tejas field; Hold the field-energy band` -- bump with provided changelog items
...
Version notation `A.B.C`: `A` = roadmap version (v0→0 … v6→6), `B` = phase within that version (`v0.2`→B=2), `C` = post-release fix on that phase. So roadmap phase `vA.B` → semver `A.B.0`; a fix after it bumps `C` (e.g. v0.3 → `0.3.0`, a follow-up fix → `0.3.1`; v1.1 → `1.1.0`). Releases are cut per phase (see [spec/ROADMAP.md](../../../spec/ROADMAP.md) §Versioning). **Never change the version without explicit user confirmation.**
...
5. Sanity-check readiness: the phase's smoke test and the determinism harness pass, and the phase's **Implementation plan** checklist in `spec/ROADMAP.md` is fully ticked -- if not, surface it and ask before proceeding
...
3. Summarize the changes into concise bullet points (group related commits; reference the NADI-xxx issues and the roadmap phase `vA.B` where relevant)
...
```bash
git tag -a v<version> -m "<one-line summary of the release, e.g. the phase Goal from ROADMAP>"
```
...
- **Don't modify source files.** This skill only touches version metadata (VERSION, README.md, RELEASE.txt), never `nadiloka/`, `species/`, `demos/`, `web/`, or `tests/` code.
- **Confirm changelog.** If auto-generating changelog from commits, show it to the user before committing.
- **Plain-text release notes.** Keep `RELEASE.txt` plain text. No emoji (project convention).
